...
class IssuesCollectionDTO(BaseModel):
    sprint: int
...
def get_issues_collection(
    jira_api: JiraAPI, jql: JQLStatements, sprint: int
) -> IssuesCollectionDTO:
    jql_planned = jql.issues_planned.format(sprint=sprint)
    jql_unplanned = jql.issues_unplanned.format(sprint=sprint)

    with ThreadPoolExecutor() as executor:
        thread_planned_issues = executor.submit(jira_api.run_jql, jql_planned)
        thread_unplanned_issues = executor.submit(jira_api.run_jql, jql_unplanned)

        issues_collection = IssuesCollectionDTO(
            sprint=sprint,
